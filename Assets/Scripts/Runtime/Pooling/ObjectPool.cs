    ...
    public class ObjectPool
    {
        ...
        private IPoolable CreateObject()
        {
            ...
            void SetPoolParent()
            {
                if (!PoolParent)
                {
                    Transform parent = new GameObject(poolObject.ObjectType.ToString()).transform;
        ...
        public IPoolable Pull(Transform parent = null, Vector3? position = null, Vector3? scale = null, Vector3? rotation = null)
        {
            ...
            {
                Debug.LogError(("BUG: Trying to add the same object ->", poolableObj.ObjectType));
            }
            return poolableObj;
        }
        ...
        public void Push(IPoolable obj,bool removeFromActiveObjects=true)
        {
            ...
            {
                Debug.LogError(("BUG: Trying to add the same object ->", obj.ObjectType));
                return;
            }
            objects.Push(obj);
